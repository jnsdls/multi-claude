...
describe("mergeUsageBody", () => {
  const stored = body({
    five_hour: win(40, iso(2 * H)),
    seven_day: win(10, iso(72 * H)),
    limits: [scoped("Opus", 12, iso(72 * H))],
  });
  const cases: [string, UsageBody | null, UsageBody, UsageBody][] = [
    ...
    [
      ...
      stored,
      body({
        five_hour: win(45, iso(2 * H)),
        seven_day: win(11, iso(72 * H)),
        limits: [scoped("Opus", 13, iso(72 * H))],
      }),
      body({
        five_hour: win(45, iso(2 * H)),
        seven_day: win(11, iso(72 * H)),
...
describe("normalizeBody", () => {
  ...
  test("a missing limits list is an empty one; missing extra_usage is disabled", () => {
    expect(normalizeBody({ five_hour: win(1, iso(H)) })).toEqual({
      five_hour: win(1, iso(H)),
      seven_day: null,
      limits: [],
      extra_usage: { is_enabled: false, spend_limit_reached: null },
    });
...
describe("isHollowBody", () => {
  const cases: [string, UsageBody, boolean][] = [
    [
      "all zero with no Reset",
      body({ five_hour: win(0, null), seven_day: win(0, null), limits: [scoped("Opus", 0, null)] }),
      true,
    ],
