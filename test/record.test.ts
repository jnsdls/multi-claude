...
function rec(o: Partial<AccountRecord> = {}): AccountRecord {
  return {
    ...
    disabled: false,
    identity: { accountUuid: "u", organizationUuid: "o", email: "e", organizationName: "n", subscriptionType: "max", capturedAt: "2026-01-01T00:00:00.000Z" },
...
const body = (n: number) => ({ five_hour: { utilization: n, resets_at: null }, seven_day: null, limits: [], extra_usage: { is_enabled: false } });
...
describe("mergeRecord", () => {
  ...
  test("a newer fetchedAt is kept over an older one", () => {
    const current = rec({ usage: { lastGood: body(50), fetchedAt: "2026-01-01T00:10:00.000Z", lastAttemptAt: null, backoffUntil: null, last429At: null } });
    const next = rec({ alias: "renamed", usage: { lastGood: body(10), fetchedAt: "2026-01-01T00:05:00.000Z", lastAttemptAt: "x", backoffUntil: null, last429At: null } });
  ...
  test("a newer reading replaces an older one", () => {
    const current = rec({ usage: { lastGood: body(50), fetchedAt: "2026-01-01T00:05:00.000Z", lastAttemptAt: null, backoffUntil: null, last429At: null } });
    const next = rec({ usage: { lastGood: body(60), fetchedAt: "2026-01-01T00:10:00.000Z", lastAttemptAt: null, backoffUntil: null, last429At: null } });
  ...
  test("an older reportedAt never replaces a Limit", () => {
    const current = rec({ lastLimit: { reportedAt: "2026-01-01T01:00:00.000Z", sessionId: "s1", window: "five_hour", resetsAt: null } });
    const next = rec({ lastLimit: { reportedAt: "2026-01-01T00:30:00.000Z", sessionId: "s0", window: null, resetsAt: null } });
  ...
  test("a newer Limit replaces an older one", () => {
    const current = rec({ lastLimit: { reportedAt: "2026-01-01T00:30:00.000Z", sessionId: "s0", window: null, resetsAt: null } });
    const next = rec({ lastLimit: { reportedAt: "2026-01-01T01:00:00.000Z", sessionId: "s1", window: null, resetsAt: null } });
    expect(mergeRecord(current, next).lastLimit?.sessionId).toBe("s1");
  });
});
